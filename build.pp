...
(def (compile-ocaml source flags)
  (effect
    ...
    (string-append (source source 0 (- (string-length source) 3)) ".cmo")))

;; The full build DAG — just a Lisp expression
(def (build-pp)
  (let [types_cmo   (delay (compile-ocaml "src/types.ml"   "-c"))
        hasher_cmo  (delay (compile-ocaml "src/hasher.ml"  "-c"))
        reader_cmo  (delay (compile-ocaml "src/reader.ml"  "-c"))
        caps_cmo    (delay (compile-ocaml "src/capabilities.ml" "-c"))
        prims_cmo   (delay (compile-ocaml "src/primitives.ml" "-c"))
        eval_cmo    (delay (compile-ocaml "src/evaluator.ml" "-c"))
        cache_cmo   (delay (compile-ocaml "src/cache.ml"     "-c"))
        repl_cmo    (delay (compile-ocaml "src/repl.ml"      "-c"))
        main_cmo    (delay (compile-ocaml "src/main.ml"      "-c"))]
