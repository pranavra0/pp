...
rekey_battery() {  # EXT (".pp" or ".ppl") -> asserts, using $TMP/v1$EXT/v2$EXT
  ...
  "$PP" "$TMP/v1$ext" > "$TMP/o1" 2>&1
  assert "$ext-v1-cold-miss-computes" "$TMP/o1" "COMPUTE" present
  assert "$ext-v1-cold-value"         "$TMP/o1" "^1$"     present

  "$PP" "$TMP/v1$ext" > "$TMP/o1b" 2>&1
  assert "$ext-v1-rerun-hit-no-compute" "$TMP/o1b" "COMPUTE" absent
  assert "$ext-v1-rerun-value"          "$TMP/o1b" "^1$"     present

  "$PP" why "$TMP/v1$ext" > "$TMP/why1" 2>&1
  assert "$ext-v1-why-reports-hit" "$TMP/why1" "\[why\].*hit" present

  # Same call site, EDITED macro definition: the node's expanded code hash
  # changes (SPEC law 20), so this MUST miss and recompute.
  "$PP" "$TMP/v2$ext" > "$TMP/o2" 2>&1
  assert "$ext-v2-edit-miss-computes" "$TMP/o2" "COMPUTE" present
  assert "$ext-v2-edit-value"         "$TMP/o2" "^2$"     present

  "$PP" why "$TMP/v2$ext" > "$TMP/why2" 2>&1
  assert "$ext-v2-why-reports-hit-after-recompute" "$TMP/why2" "\[why\].*hit" present

  # Revert to v1: the ORIGINAL key is still in the store, so this hits
  # again — no recompute.
  "$PP" "$TMP/v1$ext" > "$TMP/o3" 2>&1
  assert "$ext-v1-revert-hit-no-compute" "$TMP/o3" "COMPUTE" absent
  assert "$ext-v1-revert-value"          "$TMP/o3" "^1$"     present
}
...
# --- macro-in-node-body: an ordinary unbound-symbol error ---
cat > "$TMP/innode.pp" <<'EOF'
print(force(node { defmacro m(x) { x } }))
EOF
"$PP" "$TMP/innode.pp" > "$TMP/inn-out" 2>&1
