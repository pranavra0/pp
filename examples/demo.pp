...
(def-fexpr (my-if cond then else)
  (if (force cond) 
      (force then) 
      (force else)))
...
(let* [ro-cap  (filesystem "/etc" :ro)
       rw-cap  (filesystem "/tmp" :rw)
       net-cap (network :tcp)]
  ...
  (let [all-caps (cap-compose ro-cap net-cap)]
    (print "composed:" all-caps))
...
(def (read-config path)
  (effect :capabilities [(filesystem path :ro)]
    (print "would read config from" path)
    ;; (perform read-file path)
    (string-append "config for " path)))
