...
(def args (argv))
(assert (= (length args) 2) "gen-cproject.pp expects: <N> <src-dir>")
(def n (string->number (nth 0 args)))
(def src (nth 1 args))
(def (path f) (string-append src "/" f))

(def names (map (fn (i) (string-append "f" (number->string i))) (range 0 n)))

(perform write-file (path "shared.h")
  "#ifndef SHARED_H\n#define SHARED_H\n#define SHARED_CONST 7\n#endif\n")

(each (fn (i)
        (perform write-file (path (string-append "f" (number->string i) ".c"))
          (string-append
            "#include \"shared.h\"\n"
            "int f" (number->string i)
            "(void) { return SHARED_CONST + " (number->string i) "; }\n")))
      (range 0 n))

(perform write-file (path "sources.txt")
  (string-append (string-join "\n" names) "\nmain\n"))

(perform write-file (path "main.c")
  (string-append
    "#include \"shared.h\"\n"
    (string-join "\n" (map (fn (nm) (string-append "int " nm "(void);")) names))
    "\nint main(void) { int s = 0;\n"
    (string-join "\n" (map (fn (nm) (string-append "  s += " nm "();")) names))
    "\n  return s == 0 ? 1 : 0; }\n"))
