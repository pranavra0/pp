...
(def (greeter-build-node greeter-c)
  (node
    (do (perform run-dep "greeter.d" "cc" "-O2" "-MD" "-MF" "greeter.d"
          greeter-c "-o" "greeter")
        (blob (slurp "greeter")))))
...
(def (config-node host fixtures-root secrets-root)
  (node
    (let [greeting (string-trim
                     (slurp (string-append fixtures-root "/" host "/greeting.txt")))
          secret (slurp (string-append secrets-root "/" host "/secret.key"))
          key-tag (hash-string (unseal secret))]
...
(def (host-desired host bin-ref config hosts-root run-root)
  (let [root (string-append hosts-root "/" host)
        rundir (string-append run-root "/" host)]
    {"fs" {"bin/greeter" (string-append bin-ref ":x")
           "etc/greeter.conf" config}
     "proc" {"greeter" {"cmd" (string-append root "/bin/greeter")
                         "args" [(string-append root "/etc/greeter.conf")
                                 (string-append rundir "/status")]
                         "cwd" rundir
                         "env" {"CONFIG_HASH" (hash-string config)}}}}))

(let [greeter-c (nth 0 (argv))
      fixtures-root (nth 1 (argv))
      secrets-root (nth 2 (argv))
      hosts-root (nth 3 (argv))
      run-root (nth 4 (argv))
      hostnames (drop 5 (argv))
      ...
      batch (force-deep
              (cons (greeter-build-node greeter-c)
                    (map (fn (h) (config-node h fixtures-root secrets-root)) hostnames)))
      bin-ref (car batch)
      configs (cdr batch)]
  (foldl (fn (acc pair)
           (map-insert acc (car pair)
             (host-desired (car pair) bin-ref (cdr pair) hosts-root run-root)))
         {}
