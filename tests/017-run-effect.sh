...
#
#   (run! CMD ARG...) executes a process and returns
...
cat > "$TMP/b.pp" <<'EOF'
log!(hash-map-get(run!("echo", "hi"), "out"))
print(hash-map-get(run!("sh", "-c", "exit 3"), "exit"))
EOF
...
cat > "$TMP/c.pp" <<EOF
force(node { run!("sh", "-c", "cat $SRC/in.txt") })
EOF
...
cat > "$TMP/e1.pp" <<EOF
force(node { write!("$TMP/evil.txt", "X") })
EOF
cat > "$TMP/e2.pp" <<EOF
write!("$TMP/ok.txt", "X")
EOF
