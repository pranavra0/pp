...
let (lazy-list = map(fn(x) {
  print("  computing", x)
  x * 2
}, list(10, 20, 30, 40, 50))) {
  print("first element:", force(car(lazy-list))); print("(only one computing message should appear)") }
print("")
print("=== 3. filter ===")
print("filter even? [1 2 3 4 5 6] =>", filter(
fn(x) { x mod 2 = 0 }, list(1, 2, 3, 4, 5, 6)))

print("")
print("=== 4. foldl ===")
print("foldl + 0 [1 2 3 4 5] =>", foldl(+, 0, list(1, 2, 3, 4, 5)))
print("foldl string-append \"\" [\"a\" \"b\" \"c\"] =>", foldl(string-append, "", list("a", "b", "c")))
