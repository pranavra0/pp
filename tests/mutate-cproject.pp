...
(def args (argv))
(assert (>= (length args) 3) "mutate-cproject.pp expects: <op> <src-dir> <arg>")
(def op (nth 0 args))
(def src (nth 1 args))
...
(if (= op "edit-tu")
    (let [i (nth 2 args)]
      (perform write-file (path (string-append "f" i ".c"))
        (string-append
          "#include \"shared.h\"\n"
          "int f" i "(void) { return SHARED_CONST + " i " + 1000; }\n")))
    (if (= op "append-comment")
        (let [f (nth 2 args)]
          (perform write-file (path f)
            (string-append (slurp (path f)) "\n/* comment only */\n")))
        (assert false (string-append "unknown op: " op))))
