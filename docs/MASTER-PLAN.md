...
> `pragmatic-suffix-conventions` @ `5a7d718` (A1–A7 + A′1 committed), plus the
> A′2/A′3/A′4 work and the first slice of A′5 living in the working tree
> (uncommitted): `src/surface_tables.ml` gains a `render_spec_tables` renderer;
> `pp --dump-surface-tables` and SPEC §B.8's generated block + drift test
> (`tests/067`); the `needs` value-open differential test (`tests/068`); the
> soundness-kernel + surface `.mli` seams (`cell`, `hasher`, `capabilities`,
> `runtime`, `store`, `surface_tables`, `desugar`); and the shared
> `with-handler` pair parser with `tests/069`. Any edit to a status entry must
> update this pin. A status table that doesn't name its ref is a defect (this
> bit a reviewer once already).
...
| A′5 | **One sub-grammar, two contexts** | De-duplicate `reader_braces.ml` (2,041 lines): the normal and `quasiquote{}` paths share near-verbatim parses (e.g. handler pairs at ~778 and ~1359 on master). Factor each form's parser into a single function parameterized by context so a form added once exists in both — making A3's CI rule a backstop rather than the mechanism. | partial — the cited handler-pair loop is now ONE function, `parse_handler_pairs ps ~parse_value` (a `hname` return type each reader maps: normal keeps the string; qq builds `'sym`/`'(quote :kw)`), replacing the two ~18-line copies at the `with-handler` arms. Folding them surfaced (and fixed) a real drift: the qq copy silently accepted a trailing comma while the normal copy rejected it like every other comma list — both now reject it. Pinned by `tests/069-handler-pair-dedup.sh`; behavior-preservation confirmed by a pre/post stash diff and the 054 fuzzer + 055 fmt whole-tree gates. **Remaining:** the precedence-climbing spine (`parse_pipe/or/and/cmp/add/mul/postfix/args` vs the `parse_qq_*` mirror) and the `primary`/`head` layers — though the operator *sets* (`infix_add`/`infix_mul`/`infix_cmp`) are already shared, and A′1 already unified the `$`-head *lowering* via the `tmpl` DSL, so the residual is structural duplication (the climbing skeleton + block/with-caps arms), not closed-set drift. Not gated by the Phase A′ exit criteria (which are A′1–A′4). |
