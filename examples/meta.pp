...
def env-lookup(env, name) {
  if nil?(env) { nil } else {
# not found
    let* (pair = car(env), k = car(pair), v = cdr(pair)) {
      if k = name { v } else { env-lookup(cdr(env), name) }
    }
  }
}


def env-extend(env, name, value) { cons(cons(name, value), env) }


# Build initial environment from a list of (name . value) pairs
def env-from-pairs(pairs) { pairs }
...
def make-closure(params, body, env) { [quote { closure }, params, body, env] }


def closure?(v) {
  if vector?(v) { vector-get(v, 0) = quote { closure } } else { false }
}
...
def make-closure(params, body, env) {
  list(quote { closure }, params, body, env)
}
def closure?(v) {
  if if pair?(v) { not(nil?(v)) } else { false } { car(v) = quote { closure } } else {
    false
  }
}
...
def sym?(v) {
  if if pair?(v) {
    if car(v) = quote { (quote) } { pair?(cdr(v)) } else { false }
  } else { false } { true } else { false }
}
...
def self-evaluating?(v) {
  if nil?(v) { true } else if bool?(v) { true } else if int?(v) { true } else if float?(v) {
    true
  } else if string?(v) { true } else { keyword?(v) }
}



# Is an expression a list (pair or nil)?
def list-expr?(e) { pair?(e) }
...
# User-defined closure: extend env with params → args, eval body
    let* (params = closure-params((fn)), body = closure-body((fn)), env = closure-env((fn))) {
      eval-meta(body, extend-env-list(env, params, args))
    }
  } else { apply-host((fn), args) }
...
def extend-env-list(env, params, args) {
  if nil?(params) { env } else {
    extend-env-list(env-extend(env, car(params), car(args)), cdr(params), cdr(args))
  }
}
...
# So we use a dispatch table for known builtins.
  if (fn) = + { car(args) + car(cdr(args)) } else if (fn) = - {
    car(args) - car(cdr(args))
  } else if (fn) = * { car(args) * car(cdr(args)) } else if (fn) = / {
    car(args) / car(cdr(args))
  } else if (fn) = < { car(args) < car(cdr(args)) } else if (fn) = > {
    car(args) > car(cdr(args))
  } else if (fn) = = { car(args) = car(cdr(args)) } else if (fn) = cons {
    cons(car(args), car(cdr(args)))
  } else if (fn) = car { car(car(args)) } else if (fn) = cdr { cdr(car(args)) } else if (fn) = list {
    args
  } else if (fn) = print { print("PRINT-from-meta:", args) } else if (fn) = nil? {
    nil?(car(args))
  } else if (fn) = pair? { pair?(car(args)) } else if (fn) = int? {
    int?(car(args))
  } else if (fn) = string? { string?(car(args)) } else if (fn) = bool? {
    bool?(car(args))
  } else if (fn) = vector? { vector?(car(args)) } else if (fn) = string-append {
    apply-string-append(args)
  } else { error(string-append("unknown builtin: ", car(args))) }
}
...
# Self-evaluating: numbers, strings, nil, booleans, keywords
  if self-evaluating?(expr) { expr } else if symbol?(expr) {
    let (val = env-lookup(env, expr)) {
# Symbol: look up in environment
      if nil?(val) { error(string-append("unbound: ", expr)) } else {
        force(val)
      }
    }
# Force the value (call-by-need: env holds thunks)
  } else if list-expr?(expr) {
# It's a list: check the car for special forms
    let* (car-expr = car(expr)) {
      if car-expr = quote { (if) } { eval-if(expr, env) } else if car-expr = quote { (def) } {
        eval-def(expr, env)
      } else if car-expr = quote { (fn) } { eval-fn(expr, env) } else if car-expr = quote { (let) } {
        eval-let(expr, env)
      } else if car-expr = quote { (let*) } { eval-let*(expr, env) } else if car-expr = quote { (quote) } {
        eval-quote(expr)
      } else if car-expr = quote { (force) } { eval-force(expr, env) } else if car-expr = quote { (delay) } {
        eval-delay(expr, env)
      } else if car-expr = quote { (do) } { eval-do(expr, env) } else if car-expr = quote { def-fexpr } {
        eval-def-fexpr(expr, env)
      } else { eval-apply(expr, env) }
# Function application: (fn arg1 arg2 ...)
    }
# Not a list — error
  } else { error(string-append("cannot evaluate: ", expr)) }
}
...
def eval-if(expr, env) {
  let* (cond-expr = car(cdr(expr)), then-expr = car(cdr(cdr(expr))), else-expr = car(cdr(cdr(cdr(expr))))) {
    if force(eval-meta(cond-expr, env)) { eval-meta(then-expr, env) } else {
...
def eval-def(expr, env) {
  let* (name = car(cdr(expr)), value = car(cdr(cdr(expr)))) {
    list(quote { def-result }, name, eval-meta(value, env), env)
...
def eval-fn(expr, env) {
  let* (params = car(cdr(expr)), body = car(cdr(cdr(expr)))) {
    make-closure(params, body, env)
  }
}
# (let (name expr ...) body) — parallel
def eval-let(expr, env) {
  let* (bindings = car(cdr(expr)), body = car(cdr(cdr(expr))), new-env = extend-env-with-thunks(env, bindings)) {
...
def extend-env-with-thunks(env, bindings) {
  if nil?(bindings) { env } else {
    let* (pair = car(bindings), name = car(pair), val-e = car(cdr(pair))) {
      extend-env-with-thunks(env-extend(env, name, delay(eval-meta(val-e, env))), cdr(bindings))
    }
  }
}
...
def eval-let*(expr, env) {
  let* (bindings = car(cdr(expr)), body = car(cdr(cdr(expr)))) {
    eval-meta(body, extend-env-seq(env, bindings))
  }
}
def extend-env-seq(env, bindings) {
  if nil?(bindings) { env } else {
    let* (pair = car(bindings), name = car(pair), val-e = car(cdr(pair)), val = delay(eval-meta(val-e, env))) {
...
# (quote expr)
def eval-quote(expr) { car(cdr(expr)) }


# (force expr)
def eval-force(expr, env) {
  let (inner = car(cdr(expr))) { force(eval-meta(inner, env)) }
}

# (delay expr)
def eval-delay(expr, env) {
  let (inner = car(cdr(expr))) { delay(eval-meta(inner, env)) }
}

# (do exprs...)
def eval-do(expr, env) { let (exprs = cdr(expr)) { eval-do-list(exprs, env) } }



def eval-do-list(exprs, env) {
  if nil?(cdr(exprs)) { force(eval-meta(car(exprs), env)) } else {
...
def eval-def-fexpr(expr, env) {
  let* (name = car(cdr(expr)), params = car(cdr(cdr(expr))), body = car(cdr(cdr(cdr(expr))))) {
    list(quote { def-fexpr-result }, name, params, body, env)
  }
}

# (fn arg1 arg2 ...) — function application
def eval-apply(expr, env) {
  let* (fn-expr = car(expr), arg-exprs = cdr(expr), fn-val = force(eval-meta(fn-expr, env)), arg-thunks = map-meta(


# Create thunks for each argument (lazy!)
fn(e) { delay(eval-meta(e, env)) }, arg-exprs)) {
...
def map-meta(f, lst) {
  if nil?(lst) { nil } else { cons(f(car(lst)), map-meta(f, cdr(lst))) }
}
...
let (env = list(cons(quote { x }, delay(10)), cons(quote { y }, delay(20)))) {
  print("Lookup x:", force(env-lookup(env, quote { x })))
  print("Lookup y:", force(env-lookup(env, quote { y })))
}
...
let (env = nil) {
  print("Result:", force(eval-meta(list(quote { (if) }, true, 1, 2), env)))
}
print("")
print("=== Meta-eval'ing (let (x 42) x) ===")
let (env = nil) {
  print("Result:", force(eval-meta(list(quote { (let) }, list(list(quote { x }, 42)), quote { x }), env)))
}
print("")
print("=== Meta-eval'ing ((fn (x) (+ x 1)) 5) ===")
let (env = nil) {
# We need + in the env. For the meta evaluator, we can put host builtins.
  let (env = env-extend(env, quote { + }, +)) {
    print("Result:", force(eval-meta(list(list(quote { (fn) }, list(quote { x }), list(quote { + }, quote { x }, 1)), 5), env)))
  }
}
