...
| D23 | Module scope and top-level `let` agree between backends | Fixed. In the VM, top-level `let` bindings were stored as globals, and inline `module { ... }` bodies resolved non-local names via `LOAD_GLOBAL`, letting them see enclosing `let` bindings that the tree-walker (which evaluates modules in a fresh `initial_env()`) correctly hides. Fixed by adding an `in_module` flag to the compiler: when compiling a module body, any `ESymbol` that resolves to a non-builtin global is emitted as a new `UNBOUND` opcode instead of `LOAD_GLOBAL`, producing the same "unbound symbol" error at runtime. Pinned by differential runs; the tree-walker remains the oracle. |
| D24 | VM dynamic-extent scoping survives OCaml exceptions | Fixed. WITH_HANDLER, WITH_CONFIG, and WITH_CAPS all wrap their body in `try run_isolated … with effect …`, so every exit (normal return, OCaml exception, or effect) restores the prior extent scope. OCaml 5's effect-handler runtime owns the cleanup, replacing the manual push/pop global stacks that could leak. |
