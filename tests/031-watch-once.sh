...
rm -rf "$TMP/.pp"
echo "W1" > "$TMP/a.c"
timeout -k 5 12 "$PP" --watch --watch-interval 0.3 --grant "fs:$TMP:ro" \
...
EOF
printf 'one\n' > "$TMP/lifecycle-phase"
timeout -k 5 12 "$PP" --watch --watch-interval 0.1 --grant "fs:$TMP:ro" \
...
EOF
timeout -k 5 12 "$PP" --watch --watch-interval 0.1 --grant "fs:$TMP/world:ro" \
