...
# case does the outside file's contents reach stdout. Both backends must agree.
set -uo pipefail
PP=${PP:-bin/pp}
...
# A read that must be DENIED/fail on both backends and must never exfiltrate.
deny() {  # NAME  EXPR
  ...
  for bk in "" "--bytecode"; do
    ...
    if printf '%s' "$out" | grep -q "$SENTINEL"; then
      echo "FAIL $name${bk:+ ($bk)}: EXFILTRATED the outside file"; echo "     $out"; bad=1
    elif printf '%s' "$out" \
         | grep -qiE "permission denied|no read access|no read or write|symbolic links|error"; then
      : # denied or failed closed — defeated
    else
...
pc=0
for bk in "" "--bytecode"; do
  out=$("$PP" $bk --grant "fs:$SB:ro" -e "print(\$file(\"$SB/ok.txt\"))" 2>&1 || true)
  printf '%s' "$out" | grep -q "inside-ok" || { echo "FAIL positive-control${bk:+ ($bk)}: $out"; pc=1; }
done
