...
(def examples
  (list ;; ch 1-2: introduction + the language in brief
        (list "hello" "pp" "")
        (list "type-error" "pp" "")
        (list "lang-bindings" "pp" "")
        (list "lang-functions" "pp" "")
        (list "lang-laziness" "pp" "")
        (list "lang-effects" "pp" "")
        (list "lang-config" "pp" "")
        ;; ch 3-4: nodes + store/traces
        (list "node-identity" "pp" "")
        (list "node-freevars" "pp" "")
        (list "node-reuse" "sh" "")
        (list "caching" "sh" "")
        (list "store-layout" "sh" "")
        (list "store-rebuild" "sh" "")
        ;; ch 5: building
        (list "build-incremental" "sh" "")
        ;; ch 6-7: capabilities/secrets + modules/islands
        (list "cap-read" "pp" "")
        (list "cap-grant" "sh" "")
        (list "cap-secret" "sh" "")
        (list "mod-module" "pp" "")
        (list "mod-load-module" "sh" "")
        (list "mod-island" "sh" "")
        ;; ch 8-9: domains + distribution
        (list "domain-reconcile" "sh" "")
        (list "dist-by-hash" "sh" "")
        ;; ch 10: back ends
        (list "be-bytecode" "sh" "")
        (list "be-diff" "sh" "")
        ;; appendix A: language reference
        (list "ref-atoms" "pp" "")
        (list "ref-collections" "pp" "")
        (list "ref-arith" "pp" "")
        (list "ref-compare" "pp" "")
        (list "ref-bool" "pp" "")
        (list "ref-let" "pp" "")
        ...
        (list "ref-if" "pp" "")
        (list "ref-types" "pp" "")
        (list "ref-type-error" "pp" "")
        (list "ref-quote" "pp" "")
        (list "ref-delay" "pp" "")
        (list "ref-perform" "pp" "")
        (list "ref-config" "pp" "")
        (list "ref-defmacro" "pp" "")
        (list "ref-module" "pp" "")
        (list "ref-list-ops" "pp" "")
        (list "ref-map-ops" "pp" "")
        (list "ref-string-ops" "pp" "")
        (list "ref-predicates" "pp" "")
        (list "ref-list-stdlib" "sh" "")
        (list "ref-map-stdlib" "sh" "")
        (list "ref-string-stdlib" "sh" "")))
...
(def (run-one manual-dir pp-bin spec)
  (let [name (car spec)
        kind (car (cdr spec))
        grants (car (cdr (cdr spec)))
        gflag (if (= grants "") "" (string-append " " grants))
        is-sh (= kind "sh")
        ext (if is-sh ".sh" ".pp")
        abs-file (string-append manual-dir "/examples/" name ext)
        ;; cwd = examples/ so pp reports "NAME.pp" (never a machine path); the
        ;; captured output stays reproducible across machines.
        real-cmd (if is-sh
                   (string-append "cd " manual-dir "/examples && PP=" pp-bin
                                  " sh " name ".sh")
                   (string-append "cd " manual-dir "/examples && "
                                  pp-bin gflag " " name ".pp"))
        captured (node
                   (do (slurp abs-file)
                       (let [r (perform run "sh" "-c" real-cmd)]
                         (string-append (hash-map-get r "out")
                                        (hash-map-get r "err")))))]
    (do (if is-sh nil
          (perform materialize-file
            (string-append manual-dir "/captured/" name ".cmd")
            (string-append "pp" gflag " " name ".pp")))
        (perform materialize-file
          (string-append manual-dir "/captured/" name ".out") captured))))

;; ---- typst as hermetic tool nodes -------------------------------------------
(def (render-pdf src)
  (node
    (do (perform run-dep "pdf.d" "typst" "compile"
          "--deps" "pdf.d" "--deps-format" "make" src "pp-manual.pdf")
        (blob (slurp "pp-manual.pdf")))))

(def (render-html src)
  (node
    (do (perform run-dep "html.d" "typst" "compile"
          "--features" "html" "--format" "html"
          "--deps" "html.d" "--deps-format" "make" src "index.html")
        (blob (slurp "index.html")))))

(let [manual-dir (car (argv))
      pp-bin (car (cdr (argv)))
      src (string-append manual-dir "/manual.typ")]
  (do
    ;; 1. run every example, writing captured/*.{cmd,out}
    (force-deep (map (fn (s) (run-one manual-dir pp-bin s)) examples))
    ;; 2. typeset (typst reads captured/* + chapters via read()/include)
    (let [pdf-ref (render-pdf src)
          html-ref (render-html src)]
      (do (perform materialize-file
            (string-append manual-dir "/site/pp-manual.pdf") (blob-get pdf-ref))
          (perform materialize-file
            (string-append manual-dir "/site/index.html") (blob-get html-ref))
