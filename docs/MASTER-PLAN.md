...
> `pragmatic-suffix-conventions` @ `aca3ddf` (A1–A7 + A′1 committed at
> `5a7d718`; A′2/A′3/A′4 and the handler-pair slice of A′5 committed at
> `aca3ddf`), plus the A′5 precedence-climbing-spine dedup landing in *this*
> commit: `src/reader_braces.ml`'s two mirror spines (`parse_pipe/or/and/cmp/
> add/mul/postfix` and the `parse_qq_*` copy) collapse into one set of
> `climb_*` functions parameterized by a `spine` ops record, with
> `normal_spine`/`qq_spine` supplying the per-context builders and newline
> rules, and `parse_arglist` shared by both argument lists. Folding closed a
> drift: the qq spine had no comparison-chaining check, so `a < b < c` inside
> `quasiquote{}` died with a confusing downstream error instead of the normal
> reader's "comparison operators do not chain" — both now reject it
> identically. Any edit to a status entry must update this pin. A status table
> that doesn't name its ref is a defect (this bit a reviewer once already).
